  ...
  try {
    ...
    if (image) {
      ...
      if (!upload) {
        return res
          .status(500)
          .json(new ApiResponse(500, {}, "failed to upload on cloudinary"));
      }
      newUser.image = { url: upload.secure_url }; 
...
export const login = async (req, res) => {
  try {
    ...
    const doesThePassWordMatch = await ComparePassword(
      password,
      isThereAUserWithThisEmail.password
    );
