...
const CoachingPlan = () => {
  ...
  useEffect(() => {
    const fetchBoughtPlan = async () => {
      setLoading(true);
      try {
        const res = await axios.get(`http://localhost:8000/api/coach/getUserBoughtCourses/${auth?.user?._id}`)
  ...
  const truncateId = (id) => {
    if (!id) return "N/A";
    return typeof id === 'object' ? `${id._id.substring(0, 8)}...` : `${id.substring(0, 8)}...`;
  };
    ...
    <div className="p-4">
      ...
      {loading ? (
        ...
        <div className="bg-gray-50 p-6 rounded-lg border border-gray-200 text-center">
          <p className="text-gray-600">You haven't purchased any coaching plans yet.</p>
        </div>
      )}
    </div>
  )
